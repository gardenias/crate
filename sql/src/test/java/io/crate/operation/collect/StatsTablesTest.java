...
public class StatsTablesTest extends CrateUnitTest {
    ...
    public void testSettingsChanges() {
        ...
        StatsTables stats = new StatsTables(Settings.EMPTY, nodeSettingsService, scheduler, breakerService);
        ...
        stats = new StatsTables(settings, nodeSettingsService, scheduler, breakerService);
        ...
        assertThat(stats.jobsLog.get(), Matchers.instanceOf(RamAccountingQueue.class));
    ...
    public void testLogsArentWipedOnSizeChange() {
        ...
        StatsTables stats = new StatsTables(settings, nodeSettingsService, scheduler, breakerService);

        stats.jobsLog.get().offer(new JobContextLog(new JobContext(UUID.randomUUID(), "select 1", 1L), null));

        stats.listener.onRefreshSettings(Settings.builder()
            .put(CrateSettings.STATS_ENABLED.settingName(), true)
            .put(CrateSettings.STATS_JOBS_LOG_SIZE.settingName(), 200).build());

        assertThat(stats.jobsLog.get().getQueue().size(), is(1));

        stats.operationsLog.get().offer(new OperationContextLog(
        ...
        assertThat(stats.operationsLog.get().getQueue().size(), is(1));
    }
    ...
    public void testUniqueOperationIdsInOperationsTable() {
        ...
        BlockingQueue<OperationContextLog> queue = (BlockingQueue<OperationContextLog>) stats.operationsLog.get().getQueue();
    ...
    public void testRemoveExpiredLogs() {
        ...
        TimeExpiringRamAccountingQueue<JobContextLog> queue = new TimeExpiringRamAccountingQueue<>(context);
        ...
        assertThat(queue.getQueue().size(), is(1));
        assertThat(queue.getQueue().element().id(), is(UUID.fromString("067e6162-3b6f-4ae2-a171-2470b63dff03")));
    }
}
