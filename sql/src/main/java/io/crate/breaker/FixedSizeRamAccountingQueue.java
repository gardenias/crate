...
public class FixedSizeRamAccountingQueue<T extends Estimable> extends RamAccountingQueue<T> {

    public FixedSizeRamAccountingQueue(RamAccountingContext context, int size) {
        super(new ArrayBlockingQueue<>(size), context);
        assert size > 0 : "capacity should be > 0";
    }

    @Override
    public boolean offer(T item) {
        context.addBytesWithoutBreaking(item.estimateSize());
    ...
    public T remove() {
        ...
        context.removeBytesWithoutBreaking(removed.estimateSize());
        return removed;
    }
